...
const cards = [
  { title: "Cos'e", body: 'È una malattia autoimmune scatenata dal glutine.', icon: <SparklesIcon className="h-6 w-6" /> },
        ...
        {cards.map((card, index) => (
          <div key={card.title} className="h-full">
            <AnimateOnScroll animation="fade-up" delay={index * 80}>
              <CeliachiaInfoCard icon={card.icon} title={card.title} body={card.body} className="h-full" />
            </AnimateOnScroll>
          </div>
        ))}
      </div>
