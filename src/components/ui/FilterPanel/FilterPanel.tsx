...
export function FilterPanel({ isOpen, filters, onChange, onClose }: FilterPanelProps) {
  ...
  return (
    <>
      ...
      />
      <div className="
        fixed bottom-0 left-0 right-0 z-30 rounded-t-2xl border border-border bg-white p-4 text-textDark shadow-lg
        md:absolute md:bottom-auto md:left-0 md:right-0 md:top-[calc(100%+12px)] md:rounded-2xl
      ">
        <div className="grid gap-4 md:grid-cols-3">
          <label className="rounded-xl border border-border p-3 text-sm text-textDark">
            Distanza max: <span className="font-semibold">{filters.maxDistance ?? 30} km</span>
        ...
        <div className="mt-4 flex flex-wrap justify-end gap-2">
          ...
          <button onClick={onClose} className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-primary-dark focus-visible:ring-2 focus-visible:ring-accent">
            Applica
          </button>
        </div>
      </div>
